...
function RegisterForm() {
  ...
  const nickNameCheckAxios = (e) => {
    axios
      .post("http://172.20.10.2:8080/api/v1/member/check/nickname", {
        member_nickname: username,
      })
      .then((res) => {
        const isValidNick = res.data.isExistSameNickname === false; //응답이 제대로 온 경우
  ...
  const emailCheckAxios = (e) => {
    axios
      .post("http://172.20.10.2:8080/api/v1/member/check/email", {
        member_email: email,
      })
      .then((res) => {
        const isValidEmail = res.data.isExistSameEmail === false;
